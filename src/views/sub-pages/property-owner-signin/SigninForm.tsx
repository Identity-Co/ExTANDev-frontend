...
const SigninForm = ({ toggleForm }: SignInProps) => {
    ...
    const onSubmit: SubmitHandler<FormData> = async (data: FormData) => {
        ...
        if (res && res.ok && res.error === null) {
            ...
            if(isTotalTravel == 1) {
              ...
              redirectURL = searchParams.get('redirectTo') ?? '/admin/'
            }
    ...
    // Google Login
    useEffect(() => {
      // 1. Create script element - GOOGLE
    ...
    const handleGoogleResponse = async (response) => {
      ...
      try {
        const _res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/auth/google-login`, {
          ...
          body: JSON.stringify({'token': response.credential, 'role': 'property_owner'})
        })
    ...
    const responseFacebook = async (profile) => {
      try {
        const _res = await fetch(`https://adventureapi.deepripple.com/v1/api/auth/fb-login`, {
          ...
          body: JSON.stringify({'accessToken': profile.authResponse.accessToken, 'role': 'property_owner'})
        })
        ...
        if (data?.data) {
          const res = await signIn('credentials', {
              email: data?.data.email,
              password: data?.data.email+'_'+data?.data.fbId,
              redirect: false
          })
    ...
    return (
        <>
            ...
            <div style={{ marginTop: "20px" }}>
              {/*onClick={() => signIn("facebook", { callbackUrl: "http://localhost:3001/admin" })} */}
              <Button 
