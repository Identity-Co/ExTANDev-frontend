...
import { getAllToursForSEO } from '@/app/server/tours'
...
const OurAdventureSection2 = ({ data, toursData }: { data?: []; toursData?: []; }) => {
  const [selectedCategory, setSelectedCategory] = useState('')
  const [selectedDestination, setSelectedDestination] = useState('')
  const [currentTours, setCurrentTours] = useState(toursData)
  const [currentPage, setCurrentPage] = useState(1)
  const recordsPerPage = 100000
  const totalPages = 1;
  ...
  const fetchTours = async (category: number, destination: string, page: number) => {
    ...
    const data = await getAllToursForSEO(category, destination, page)
