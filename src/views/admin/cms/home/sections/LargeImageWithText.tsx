...
const LargeImageWithText = ({ section }: { section?: [] }) => {
  ...
  const [imgSrc, setImgSrc] = useState<string>(section?.image ? `${process.env.NEXT_PUBLIC_UPLOAD_URL}/${section?.image}` : '/images/avatars/1.png')
  ...
  const handleFileInputChange = (file: ChangeEvent) => {
    ...
    const maxSize = 2048 * 1024; // 800 KB

    if (file2.size > maxSize) {
      toast.error("File is too large. Maximum allowed size is 2048KB.")
