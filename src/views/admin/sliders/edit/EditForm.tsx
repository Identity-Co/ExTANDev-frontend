...
const EditForm = ({ setId, banner }: EditProps) => {
  ...
  const handleFileInputReset = () => {
    setFileInput('')
    setImgSrc(banner.banner_image??'/images/avatars/1.png')
  }
  ...
  const onUpdate: SubmitHandler<FormData> = async (data: FormData) => {
    if (!imageInput && !banner.banner_image) {
    ...
    if (log && log._id) {
      if(imageInput) {
        const banner = await Banner.updateBannerInfo(setId, fData)
      }
