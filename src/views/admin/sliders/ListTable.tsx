...
// Type Imports
import type { BannerTypes } from '@/types/apps/bannerTypes'
...
type BannerTypesWithAction = BannerTypes & {
  action?: string
  banner_image?: string
  title?: string
  page?: string
...
// Column Definitions
const columnHelper = createColumnHelper<BannerTypesWithAction>()

const BannerListTable = ({ tableData }: { tableData?: BannerTypes[] }) => {
  ...
  const columns = useMemo<ColumnDef<BannerTypesWithAction, any>[]>(
    () => [
      ...
      columnHelper.accessor('page', {
        header: 'Page',
        cell: ({ row }) => <Typography>{row.original.page}</Typography>
      }),
      columnHelper.accessor('action', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center'>
            ...
            <SmartLink href={'/admin/banner-sliders/edit/'+row.original._id}>
  ...
  const handleDeleteAction = async () => {
    const res = await Banner.deleteBanner(userId)
  ...
  const fetchData = async () => {
    const refresh = await Banner.getAllBanner();
    
    setData(refresh);
  };

  const table = useReactTable({
    data: filteredData as BannerTypes[],
    columns,
    filterFns: {
      fuzzy: fuzzyFilter
    },
  ...
  return (
    <>
      <Card>
        <CardHeader title='Banners/Sliders' />
        ...
        <div className='flex justify-between p-5 gap-4 flex-col items-start sm:flex-row sm:items-center'>
          ...
          <div className='flex items-center gap-x-4 gap-4 flex-col max-sm:is-full sm:flex-row'>
            ...
            <SmartLink href={`/admin/banner-sliders/add/`}>
