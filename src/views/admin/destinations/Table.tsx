...
const ListTable = ({ tableData }: { tableData?: [] }) => {
  ...
  const columns = useMemo<ColumnDef<PageTypesWithAction, any>[]>(
    () => [
      ...
      columnHelper.accessor('sub_title', {
        header: 'Sub Title',
        cell: ({ row }) => <Typography>{row.original.sub_title}</Typography>
      }),
      columnHelper.accessor('action', {
        header: 'Action',
        cell: ({ row }) => (
          <div className='flex items-center'>
            <IconButton sx={{color: 'warning.main'}} href={`/admin/destination/edit/${row.original._id}`} className='flex'>
  ...
  return (
    <>
      <Card>
        <CardHeader title='Manage Destinations' />
        ...
        <div className='flex justify-between p-5 gap-4 flex-col items-start sm:flex-row sm:items-center'>
          ...
          <div className='flex items-center gap-x-4 gap-4 flex-col max-sm:is-full sm:flex-row'>
            ...
            <SmartLink href={`/admin/destination/add/`}>
