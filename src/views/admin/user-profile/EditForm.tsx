...
import { object, minLength, string, pipe, nonEmpty, email, optional, number, minValue, maxValue } from 'valibot'
...
type PasswordFormData = InferInput<typeof passwordSchema>
...
const EditForm = ({ setId, userInfo, roles }: EditProps) => {
  ...
  const onUpdate: SubmitHandler<FormData> = async (data: FormData) => {
    ...
    const f_data = {
      ...
      "socialmedia_urls" : {
        ...
        "pinterest_url" : userInfo.pinterest_url ?? '',
        "twitterx_url" : userInfo.twitterx_url ?? '',
        "whatsapp_url" : userInfo.whatsapp_url ?? '',
  ...
  const onChangePassword: SubmitHandler<PasswordFormData> = async (data: PasswordFormData) => {
    ...
    try {
      ...
      const res = await User.changePassword(_fdata);
        ...
        <CardContent>
          ...
          >
            <Grid container spacing={5}>   
              ...
              <Grid size={{ xs: 12 }} className='flex gap-4 flex-wrap' justifyContent="space-between" container>
                <Grid>
            ...
            <Grid container spacing={5}>
              ...
              <Grid size={{ xs: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center' }}>
                  ...
                  {isChangingPassword ? <CircularProgress style={{marginLeft: '8px'}} size={24} thickness={6} /> : ''}
                </div>
              </Grid>
            </Grid>
          </form>
