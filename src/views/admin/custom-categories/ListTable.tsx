...
type CategoryTypesWithAction = CategoryTypes & {
  action?: string
  _id?: string
}
...
// Column Definitions
const columnHelper = createColumnHelper<CategoryTypesWithAction>()

const CategoryListTable = ({ tableData }: { tableData?: CategoryTypes[] }) => {
  ...
  const [catId, setId] = useState<string>('')
  ...
  const columns = useMemo<ColumnDef<CategoryTypesWithAction, any>[]>(
    () => [
      columnHelper.accessor('category_name', {
        header: 'Category Name',
        cell: ({ row }) => <Typography>{row.original.category_name}</Typography>
      }),
      columnHelper.accessor('search_pattern', {
        header: 'Search Pattern',
        cell: ({ row }) => <Typography>{row.original.search_pattern}</Typography>
      }),
      columnHelper.accessor('action', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center'>
            ...
            <SmartLink href={'/admin/custom-categories/edit/'+row.original._id}>
  ...
  const handleDeleteAction = async () => {
    const res = await Category.deleteCategory(catId)
  ...
  const fetchData = async () => {
    const refresh = await Category.getAllCategories();

    setFilteredData(refresh);
  };

  const table = useReactTable({
    data: filteredData as CategoryTypes[],
    columns,
    filterFns: {
      fuzzy: fuzzyFilter
    },
  ...
  return (
    <>
      <Card>
        <CardHeader title='Tour Custom Categories' />
        ...
        <div className='flex justify-between p-5 gap-4 flex-col items-start sm:flex-row sm:items-center'>
          ...
          <div className='flex items-center gap-x-4 gap-4 flex-col max-sm:is-full sm:flex-row'>
            ...
            <SmartLink href={`/admin/custom-categories/add/`}>
      ...
      >
        <DialogTitle id='alert-dialog-title' className='text-center'>Delete Category?</DialogTitle>
        <DialogContent>
